...
Scores each facility's social media footprint across six signals — platform breadth, posting recency, post volume, follower count, likes, and engagements — producing a single **social presence score (0–10)** that reflects how active and credible the facility's online presence is.

A facility with no social media data at all scores **0**. A facility with a wide multi-platform presence, recent posts, and strong engagement scores up to **10**.
...
#### 2. Posting Recency — `post_metrics_most_recent_social_media_post_date` (0–3 pts)

Measures how recently the facility posted. The column stores ISO dates (`YYYY-MM-DD`) for the majority of records; 18 rows contain relative strings (e.g. `"2 months ago"`, `"9h"`) which are treated as **recent** (3 pts) since they were clearly scraped while active.
...
|---|---|
| NULL | 0 |
| Relative string (contains letters, not a full date) | 3 — treat as active |
...
#### 4. Follower Count — `engagement_metrics_n_followers` (0–2 pts)
...
#### 5. Likes — `engagement_metrics_n_likes` (0–1 pt)
...
#### 6. Engagements — `engagement_metrics_n_engagements` (0–1 pt)
...
| Score | Label | Meaning |
|---|---|---|
| 9–10 | Strong | Wide multi-platform presence, recent activity, large audience |
| 6–8 | Good | Active on multiple platforms with meaningful engagement |
| 3–5 | Moderate | Some social presence but limited activity or reach |
| 1–2 | Weak | Minimal presence — few platforms, old or no posts, tiny audience |
...
WITH scored AS (
  SELECT
    ...
    -- Sub-score 2: Posting recency (0–3)
    -- NULL → 0 (scrape gap vs true zero resolved via data_gap flag, not by inflating the score)
    -- Relative strings (e.g. "2 months ago", "9h") → 3 (active at crawl time)
    CASE
      ...
      WHEN TRY_CAST(post_metrics_most_recent_social_media_post_date AS DATE) IS NULL
        AND post_metrics_most_recent_social_media_post_date RLIKE '[a-zA-Z]' THEN 3
...
- `recency_score = 0 AND follower_score = 2` — large audience but no recent posts; account may be inactive or abandoned
...
- **Score does not validate account ownership**: A high score confirms social activity was observed, not that the accounts belong to this specific facility. Combine with `facebook_page_validation.md` for identity verification.
