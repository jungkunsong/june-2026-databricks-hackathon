...
> **Agent:** `WebsiteAgent`
> **Rubric role:** The SQL below is for data retrieval only. Sub-score thresholds and the final score are determined by the agent after fetching the raw fields. Treat the point values in the tables below as calibrated defaults — the agent may adjust them when context warrants (e.g. a NULL `recency_of_page_update` that is clearly a scrape gap rather than a genuinely stale profile). See [Agent Judgment Guidelines](#agent-judgment-guidelines).
...
Scores each facility's web presence across five signals — website reachability, recency of page update, affiliated staff presence, custom logo presence, and number of facts about the organization — producing a single **page presence score (0–20)** that reflects how credible and well-maintained the facility's online profile is.

A facility with no website and no page metadata scores **0**. A facility with a live website, a recently updated profile, staff listed, a custom logo, and rich factual content scores up to **20**.
...
#### 1. Website Reachability — `officialWebsite` (0–4 pts)
...
| NULL / no website | 0 |
| 000 (unreachable) | 0 |
| 4xx / 5xx (server error or client error) | 2 — domain exists but misconfigured |
| 301 / 302 (redirect) | 4 — likely valid |
| 200 (OK) | 4 |

#### 2. Recency of Page Update — `recency_of_page_update` (0–6 pts)
...
|---|---|
| NULL | 0 |
| Within last 6 months | 6 |
| 6 months – 1 year ago | 4 |
| 1 – 2 years ago | 2 |
| Older than 2 years | 0 |

#### 3. Affiliated Staff Presence — `affiliated_staff_presence` (0–4 pts)
...
#### 4. Custom Logo Presence — `custom_logo_presence` (0–2 pts)
...
#### 5. Number of Facts About the Organization — `number_of_facts_about_the_organization` (0–4 pts)
...
```
page_presence_score =
    website_reachability_score   (0–4)
  + recency_score                (0–6)
  + staff_score                  (0–4)
  + logo_score                   (0–2)
  + facts_score                  (0–4)
...
**Total range: 0–20** (capped at 20)

| Score | Label | Meaning |
|---|---|---|
| 17–20 | Strong | Live website, recently updated profile, staff listed, logo present, rich factual content |
| 12–16 | Good | Most signals present; minor gaps in recency or content depth |
| 6–11 | Moderate | Some page metadata present but website unreachable or profile stale |
| 2–5 | Weak | Minimal signals — likely a stub record |
| 0–1 | None | No website and no page metadata at all |

---

## SQL — Data Retrieval

The agent uses this query to fetch raw web presence fields. Sub-score computation and the final `page_presence_score` are determined by the agent after retrieval, not inside the SQL.
...
> **Note on website reachability:** `website_reachability_score` requires an HTTP check run outside SQL. The agent should perform or look up the HTTP status for `officialWebsite` and incorporate it into the final score using the reachability table above.
...
- **NULL on `recency_of_page_update`:** Score 0 by default. However, if other signals (staff, logo, facts) are all strong and the facility is a well-known institution, the agent may treat the NULL as a scrape gap and not penalise recency.
- **NULL on `number_of_facts_about_the_organization`:** Score 0 by default. If the facility's description or specialties fields are rich, the agent may award partial credit (1–2 pts) as a proxy for factual depth.
- **HTTP 301/302 redirects:** Score the same as 200 (4 pts) unless the redirect chain resolves to a clearly unrelated domain (e.g. a parked domain or a generic error page).
- **`officialWebsite` is a bare domain without scheme:** Normalise to `https://` before the HTTP check. Do not penalise for the missing scheme.
- **`affiliated_staff_presence = 'null'` (string):** Treat as NULL → 0 pts. Do not treat as `true`.
...
- `recency_score = 0 AND staff_score = 4` — staff are listed but the profile has never been updated or is very stale; staff list may be outdated
- `facts_score = 0 AND staff_score = 4` — staff present but no structured facts; profile is incomplete
- `logo_score = 0 AND recency_score = 6` — recently updated but no logo; may indicate an auto-populated record rather than a self-managed profile
...
- **Website reachability requires an external HTTP check:** The SQL data retrieval query omits `website_reachability_score`. The agent must perform or look up the HTTP status and incorporate it (0–4 pts) into the final score. Without it, the maximum achievable score from SQL data alone is 16.
